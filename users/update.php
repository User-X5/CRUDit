...
//get roles
     $selectRolesQuery = "SELECT roles.id,roles.name FROM roles";
    $selectRolesResult = mysqli_query($connect, $selectRolesQuery);
...
if (isset($_POST['btn'])) {
    ...
    $updateQuery = "UPDATE users SET name= '$name' , `password` = '$password' , role_id = '$role_id' WHERE id = $id";
...
<section class="w-100 bg-section py-5">
    <h1 class="text-center fw-bold text-white text-uppercase">Users Update</h1>
            ...
            <div class="col-6">
                <form action="" method="POST" class="form-container text-white fw-bold text-uppercase">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" value="<?php echo $item['name']?>" required name="name" class="form-control" id="name"">
                    </div>
                    <div class=" mb-3">
                        <label for="password" class="form-label">password</label>
                        <input type="password" value="<?php echo $item['password']?>" required name="password" class="form-control" id="password"">
                    </div>
                    <div class=" mb-3">
                        <label for="role_id" class="form-label">role</label>
                        <select class="form-select" name="role_id" id="role_id">
                            <?php foreach ($selectRolesResult as $role) { ?>
                                <option value="<?php echo $role['id'] ?>" <?php ($role['id'] === $role_id ) ?  print('selected') : '' ;?>><?php echo $role['name'] ?></option>
                            <?php } ?>
                        </select>
                    </div>
                    ...
                    <div class=" btns-form d-flex justify-content-evenly ">
                        ...
                        <button class="btn btn-primary back-button"><a href="../users/user.php" class="w-100 h-100 text-white ">Back</a></button>
                    </div>
                </form>
            </div>
        </div>
