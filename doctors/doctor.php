...
$selectQuery = mysqli_query($connect, "SELECT * FROM doctors");
...
if($authAdmin) {
    if (isset($_GET['delete'])) {
        ...
        $updateDoctorIDonStudents = mysqli_query($connect, "UPDATE students SET doctor_id = NULL WHERE doctor_id = " . $_GET['delete']);
        $deleteQuery = "DELETE FROM doctors WHERE id = " . $_GET['delete'];
        $deleteResult = mysqli_query($connect, $deleteQuery);
        if ($deleteResult) {
            header("location: /finalproject/doctors/doctor.php");
        }else {
                echo "
...
<section class="w-100 bg-section py-5">
    <h1 class="text-center fw-bold text-white text-uppercase">Doctor List</h1>
                    ...
                    <tbody>
                        <?php foreach ($selectQuery as $item) { ?>
                            <tr>
                                ...
                                <?php if (isset($_SESSION['name']) && $authAdmin) {?>
                                    <td>
                                        <a href="/finalproject/doctors/update.php?edit=<?php echo $item['id'] ?>" class="text-white link-hover fs-5"><i class="fa-solid fa-pen-to-square"></i></a>
                                        <a href="/finalproject/doctors/doctor.php?delete=<?php echo $item['id'] ?>" class="text-danger mx-2 link-hover fs-5"><i class="fa-solid fa-trash-can"></i></a>
                                    </td>
                                <?php }?>
                            </tr>
                        <?php } ?>
