...
if (isset($_GET['edit'])) {
    $edit_id = $_GET['edit'];
    $selectQuery = "SELECT * FROM courses WHERE id = $edit_id";
...
if (isset($_POST['btn'])) {
    ...
    $updateQuery = "UPDATE courses SET name= '$name' , price = $price, hours = $hours WHERE id = $id";
...
<section class="w-100 bg-section py-5">
    <h1 class="text-center fw-bold text-white text-uppercase">Course Update</h1>
        ...
        <div class="row justify-content-evenly animate__animated animate__fadeIn">
            <div class="col-6">
                <form action="" method="POST" class="form-container text-white fw-bold text-uppercase">
                    ...
                    <input type="text" name="name" required value="<?php echo $name ?>" class="form-control" id="name"">
                    </div>
                    <div class=" mb-3">
                    <label for="price" class="form-label">price</label>
                    <input type="number" name="price" required value="<?php echo $price ?>" class="form-control" id="price"">
                    </div>
                    <div class=" mb-3">
                    <label for="hours" class="form-label">hours</label>
                    <input type="hours" name="hours" required value="<?php echo $hours ?>" class="form-control" id="hours"">
                    </div>
                    

                    <div class=" btns-form d-flex justify-content-evenly ">
                        <button type=" submit" name="btn" class="btn btn-primary">Submit</button>
                    <button class="btn btn-primary back-button"><a href="../courses/course.php" class="w-100 h-100 text-white ">Back</a></button>
            </div>
            </form>
        </div>
    </div>
