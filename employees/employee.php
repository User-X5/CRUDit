...
$selectQuery = mysqli_query($connect, "SELECT employees.id , employees.age, employees.name as employee_name ,employees.salary ,
...
if($authAdmin){
    if (isset($_GET['delete'])) {
        ...
        $deleteQuery = "DELETE FROM employees WHERE id = " . $_GET['delete'];
        $deleteResult = mysqli_query($connect, $deleteQuery);
        if ($deleteResult) {
            header("location: /finalproject/employees/employee.php");
        } else {
                echo "
...
<section class="w-100 bg-section py-5">
    <h1 class="text-center fw-bold text-white text-uppercase">Employee List</h1>
    <div class="container doctor-container">
        <div class="row justify-content-center animate__animated animate__fadeIn">
            <div class="col-lg-12 col-md-6 col-sm-4 w-100">
                <table>
                    ...
                    <tbody>
                        <?php foreach ($selectQuery as $item) { ?>
                            <tr>
                                ...
                                <?php if (isset($_SESSION['name']) && $authAdmin) {?>
                                    <td>
                                        <a href="/finalproject/employees/update.php?edit=<?php echo $item['id'] ?>" class="text-white link-hover fs-5"><i class="fa-solid fa-pen-to-square"></i></a>
                                        <a href="/finalproject/employees/employee.php?delete=<?php echo $item['id'] ?>" class="text-danger mx-2 link-hover fs-5"><i class="fa-solid fa-trash-can"></i></a>
                                    </td>
                                <?php }?>
                            </tr>
                        <?php } ?>
